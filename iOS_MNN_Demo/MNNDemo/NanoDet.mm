...
NanoDet::NanoDet(bool useGPU) {
    ...
    NSString *path = [NSString stringWithCString:"nanodet_320.mnn" encoding:[NSString defaultCStringEncoding]];
    ...
    NanoDet_interpreter = std::shared_ptr<MNN::Interpreter>(MNN::Interpreter::createFromFile([model_path UTF8String]));
...
std::vector<BoxInfo>
NanoDet::detect(UIImage *image, double threshold, double nms_threshold) {
    ...
    cv::Mat dst(in_h, in_w, CV_8UC4);
    cv::Mat image_input(height, width, CV_8UC4, imageSource.get());
    cv::resize(image_input, dst, cv::Size(in_w, in_h));
    ...
    pretreat->convert(dst.data, in_w, in_h, dst.step[0], input_tensor);
